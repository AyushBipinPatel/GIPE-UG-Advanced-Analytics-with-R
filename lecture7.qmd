...
## Execute the Benjamini-Hochberg Procedure
...
all_test_pvalues|>
  arrange(Independent_t.test_pval)|>
  mutate(
    index = c(1:2000),
    L_val_bh = (0.05*index)/2000,
    is_L_bh = Independent_t.test_pval < L_val_bh
  )|>
  filter(is_L_bh == T & 
           L_val_bh == max(L_val_bh))|>
  pull(Independent_t.test_pval) -> bh_L

## reject null hypothesis where pvalues are less than or equal to L

all_test_pvalues|>
  mutate(
    reject_null_for_holms = Independent_t.test_pval < holms_L,
    reject_null_for_bh = Independent_t.test_pval <= bh_L
  ...
  summarise(
    ...
    rejected_nulls_bonferroni = sum(reject_null_for_bonferroni),
    rejected_nulls_holms = sum(reject_null_for_holms),
    rejected_nulls_bh = sum(reject_null_for_bh)
  )|>
  gt::gt()
